...
- `composer install` / vendor downloads (10–60s)
- Service restarts that may take time
...
## Installation Flow (Standard)

This section documents the standard interactive installation flow that ties together the intro screen, confirmation prompts, idempotency checks, and the main() ordering convention.

### show_intro() — Pre-install Welcome Screen (Template)

Every interactive script SHOULD display a welcome screen before installation. This clears the terminal, shows a title box, system info, installation steps, and requirements. Skipped in silent mode.

```bash
show_intro() {
    clear
    
    draw_box "AppName Installer v${SCRIPT_VERSION}"
...
**Adaptation points:**
- Change the title string in `draw_box`
- Add/remove `print_subheader` lines to match actual install steps
- Add app-specific requirements (e.g., "Cloudflare tunnel token required")
- Some scripts add domain/FQDN info if `detect_network_info()` ran earlier

### confirm_start() — Pre-install Confirmation (Template)

Gate the installation behind an explicit yes/no prompt. Bypassed in silent mode.
...
### show_already_installed() — Idempotency Menu (Template)

When the idempotency check detects an existing installation, show a management menu instead of re-installing. This is the standard output the user sees when running a script that has already completed.
...
show_already_installed() {
    ...
    print_kv "Version" "$(appname --version 2>/dev/null | head -1 || echo 'unknown')"
...
**Adaptation points:**
- Version detection varies by app (`smbd -V`, `cloudflared --version`, `dpkg-query`, etc.)
- Multi-service apps should show status for each service (see Multi-Service Status under Post-Install Commands)
- Docker Compose apps show container status instead of systemd status
- Apps with `--force` can replace the "uninstall first" hint with a `--force` hint

### Clear Screen on Interactive Launch (Convention)

Scripts clear the terminal before showing the intro screen. Use this guard:

```bash
[[ -t 1 ]] && ! is_silent && clear
```

- `-t 1`: Only if stdout is a real terminal (not piped or redirected)
- `! is_silent`: Never clear in non-interactive mode
- Some scripts call `clear` inside `show_intro()` directly — either approach is acceptable

### main() Installation Flow (Convention)

The standard `main()` follows this ordering after the post-install command dispatch:

```bash
main() {
    # 1. Post-install command dispatch (--status, --logs, etc.)
    case "${1:-}" in
        ...
        --force|-f)   FORCE_INSTALL=true ;;   # optional
        "")           ;;  # Continue with installation
        *)            die "Unknown option: $1 (use --help for usage)" ;;
    esac
    
    # 2. Early sudo check (before logging — no functions available yet)
    # ... (verbatim from Mandatory Safety Checks) ...
    
    # 3. Idempotency check
    if <app_is_already_installed>; then
        show_already_installed
        exit 0
    fi
    
    # 4. Clear screen + show intro (interactive only)
    [[ -t 1 ]] && ! is_silent && clear
    if ! is_silent; then
        show_intro
    fi
    
    # 5. Setup logging (requires sudo — checked above)
    setup_logging
    
    # 6. Pre-flight checks
    preflight_checks
    
    # 7. Interactive configuration (if any)
    get_user_configuration   # prompts for domain, port, token, etc.
    
    # 8. Confirm start (interactive only)
    if ! is_silent; then
        confirm_start
    fi
...
**Key ordering rules:**
- `--help` is handled BEFORE `main()` (at top of script, before function definitions)
- Post-install commands run before any sudo checks (they have their own guards)
- Idempotency check happens BEFORE `show_intro` and `setup_logging`
- `show_intro` and `confirm_start` are skipped in silent mode
- `setup_logging` happens after sudo is confirmed but before `preflight_checks`

### --force / Reinstall Pattern (Optional)

For apps that benefit from in-place reinstall without separate uninstall + install. Only add when the app warrants it (e.g., single-directory installs).

#### Variable and Argument

```bash
# Near top of script (after env var block)
FORCE_INSTALL=false

# In main() case dispatch:
--force|-f)   FORCE_INSTALL=true ;;
```

#### Force Confirmation (dangerous action — requires typing DELETE)
...
confirm_force_reinstall() {
    ...
    echo
    print_warning "This will DELETE the existing ${SCRIPT_NAME} installation!"
    echo
    print_info "The following will be removed:"
    echo "  ${C_RED}${SYMBOL_BULLET} ${INSTALL_DIR}${C_RESET}"
    echo "  ${C_RED}${SYMBOL_BULLET} /etc/systemd/system/${SCRIPT_NAME}.service${C_RESET}"
    echo
...
#### Force Handler (stop, remove, clean)
...
handle_force_reinstall() {
    ...
    sudo systemctl stop "$SCRIPT_NAME" 2>/dev/null || true
    sudo systemctl disable "$SCRIPT_NAME" 2>/dev/null || true
...
if check_previous_installation; then
    if [[ "$FORCE_INSTALL" == true ]]; then
        clear
        draw_box "AppName Reinstall (--force)"
        echo
        setup_logging
        preflight_checks
        confirm_force_reinstall
...
## Fresh Server / Conflict Detection (Optional)

Some apps (BookStack, full-stack installers) require a clean environment. Add these checks inside `preflight_checks()` after the standard checks.

```bash
# Inside preflight_checks(), after standard checks:

# Fresh server checks (if app requires dedicated VM/LXC)
...
if [[ -d "$INSTALL_DIR" ]]; then
    die "Install directory already exists ($INSTALL_DIR). Use --uninstall first."
fi
```

**When to add fresh-server checks:**
- App installs a web server (Apache/Nginx) and assumes it owns the config
- App installs a database server and creates its own databases
- App cannot coexist with other services on the same ports

**When NOT to add:**
- App installs into its own directory without conflicting with system services
- App uses Docker (isolated by design)
...
### Selective Package Install (Recommended)

Instead of always running `apt-get install` for all packages, check which are already present and only install what's missing. This avoids unnecessary `apt-get update` calls and is faster on re-runs.
...
install_packages() {
    ...
    if [[ ${#packages_to_install[@]} -gt 0 ]]; then
        ...
        if ! run_with_spinner "Installing packages" sudo apt-get install -y -qq "${packages_to_install[@]}"; then
...
**Key points:**
- Use `dpkg -s "$pkg"` (not `command -v`) — checks for the package, not just an executable
- Only call `apt-get update` if there are packages to install
- Log each already-present package with `print_success` for visibility
- The `packages_to_install` array is passed to a single `apt-get install` call

### Version-Gated Dependency Install (Template)

When a runtime dependency (Node.js, Python, etc.) has a minimum version requirement, check the installed version before attempting to install/upgrade:

```bash
readonly NODE_MAJOR="24"  # Minimum required major version
...
**Pattern:** Check `command_exists` → get version → compare → skip if sufficient → install/upgrade if not.
...
### GPG Key Idempotency (Convention)

When adding third-party APT repositories, always guard GPG key downloads to avoid unnecessary network requests and GPG warnings:

```bash
# Using /etc/apt/keyrings/ (modern signed-by approach)
...
if [[ ! -f /etc/apt/keyrings/vendor.gpg ]]; then
    ...
    sudo chmod a+r /etc/apt/keyrings/vendor.gpg
...
```bash
# Using /usr/share/keyrings/ (some vendors prefer this)
sudo mkdir -p --mode=0755 /usr/share/keyrings

if [[ -s "/usr/share/keyrings/vendor.gpg" ]]; then
    print_success "GPG key already present"
else
    if ! curl -fsSL https://vendor.example.com/gpg.key | \
         sudo tee "/usr/share/keyrings/vendor.gpg" >/dev/null 2>>"$LOG_FILE"; then
        die "Failed to download GPG key"
    fi
...
**Key points:**
- Check with `-f` (exists) or `-s` (exists and non-empty) before downloading
- Use `--dearmor` when the upstream key is ASCII-armored (most common)
- Use `sudo tee` when the upstream key is already binary/dearmored
- Always set `chmod a+r` on the keyring file so apt can read it

---

## GitHub Release Download Pattern (Template)

For apps installed from GitHub releases (not APT). Requires `curl` and `jq`.

```bash
readonly APP_REPO="owner/repo"

download_app() {
    print_header "Downloading AppName"
    
    print_step "Fetching latest release from GitHub..."
    
    local release_info
    if ! release_info=$(curl -fsSL "https://api.github.com/repos/${APP_REPO}/releases/latest" 2>&1); then
    ...
    # Download tarball
    local tmp_tarball="/tmp/${SCRIPT_NAME}-${version}.tar.gz"
    ...
    # Verify extraction (use an app-specific marker file)
    if [[ ! -f "${INSTALL_DIR}/package.json" ]]; then
        die "Extraction failed - expected file not found"
    fi
    
    log SUCCESS "AppName ${version} downloaded"
    echo
}
```

**Notes:**
- GitHub API rate-limits unauthenticated requests to 60/hour — the error message hints at this
- `jq` must be in the prerequisites; add it to the dependency list
- `--strip-components=1` removes the top-level directory GitHub wraps tarballs in
- For binary releases (not tarballs), use `.assets[]` filtering with `jq` instead of `.tarball_url`
...
### Config Validation Before Service Restart (Convention)

After writing a config file and before restarting the service, always run the app's config validator if one exists. Replace the validation hook comment above with the real validator:
...
if ! sudo unbound-checkconf "$target_conf" >/dev/null 2>&1; then
    die "Unbound configuration validation failed"
fi

# SSH
if ! sudo sshd -t -f /etc/ssh/sshd_config 2>&1; then
    die "SSH configuration validation failed"
fi

# Apache
if ! sudo apachectl configtest 2>&1; then
    die "Apache configuration validation failed"
fi
```

**Pattern:** validate → die on failure → restart only on success. Only restart/reload the service when `CONFIG_CHANGED` is `true` AND validation passes.
...
**WARNING**: Do NOT add `umask 077` to scripts that create shared directories, web roots, or any path requiring group/world access. `umask 077` causes `mkdir -p` to create intermediate directories as 700 — blocking traversal even if you explicitly `chmod` the final directory. Only use for scripts that generate secret files (Docker .env, API tokens, database passwords).
...
# Generate secure password (alphanumeric, exact length)
generate_password() {
    local length="${1:-35}"
    local password=""
...
### Systemd Service File Creation (Template)

Pattern for creating a systemd unit file from the installer:

```bash
create_systemd_service() {
    print_header "Creating Systemd Service"
    
    print_step "Creating ${SCRIPT_NAME}.service..."
    
    sudo tee /etc/systemd/system/${SCRIPT_NAME}.service > /dev/null << EOF
[Unit]
...
ExecStart=/usr/bin/appname --flag
Restart=always
RestartSec=10
...
**Service enable + start** (separate step, after config is complete):

```bash
enable_and_start_service() {
    print_header "Starting Service"
    
    if ! sudo systemctl enable "$SCRIPT_NAME" >/dev/null 2>&1; then
        die "Failed to enable $SCRIPT_NAME service"
    fi
    print_success "Service enabled (starts on boot)"
    
    if ! sudo systemctl start "$SCRIPT_NAME"; then
        die "Failed to start $SCRIPT_NAME service"
    fi
    
    # Verify it's running
    sleep 2
    if service_is_active "$SCRIPT_NAME"; then
        log SUCCESS "Service is running"
    else
        die "Service failed to start — check: journalctl -u $SCRIPT_NAME"
    fi
...
**Key points:**
- Uses unquoted heredoc (`<< EOF`) because variables like `${INSTALL_DIR}` must expand
- Always `daemon-reload` after creating/modifying unit files
- Separate creation from enable+start — allows config steps in between
- Verify the service is actually running after start

---

## Docker Group Membership (Standard for Docker-dependent Scripts)

Any script that installs or uses Docker must add the current user to the `docker` group:
...
**Key points:**
- Check with `id -nG` (groups by name), not `groups` command (which can be stale)
- `tr ' ' '\n' | grep -qx docker` avoids false matches (e.g., group "docker-users")
- The `newgrp docker` warning is important — group changes don't take effect in the current session
- Docker Compose commands in the same script session may still need `sudo` even after `usermod`

---

## Docker Compose App Pattern (Template)

For apps deployed via Docker Compose (as opposed to systemd services). This covers directory structure, secrets, .env file, compose file, and startup.

### Directory Structure

```bash
readonly WORK_DIR="$HOME/appname"
...
### Docker-or-Sudo Convention

Docker Compose commands may need `sudo` if the user's group membership isn't active in the current session. Repeat this check before every `docker compose` call:
...
This pattern is used consistently in `cmd_status`, `cmd_logs`, `cmd_uninstall`, and startup functions. See also the Docker Compose variants under Post-Install Commands.
...
#### Multi-Service Status Variant

When an app depends on multiple services, iterate over all of them:
...
cmd_status() {
    ...
    local services=("apache2" "php8.4-fpm" "mariadb")
...
#### Docker Compose Status Variant

For apps deployed via Docker Compose. Test if docker works without sudo first (group may not be active in current session):
...
## Optional Post-install Reboot Prompt (Template)

For apps that benefit from a reboot (kernel parameter changes, group membership, etc.) but don't require one. Call at the very end of `main()`, after `show_summary()`.

```bash
prompt_reboot() {
    if [[ "${APPNAME_SKIP_REBOOT:-false}" == "true" ]]; then
        return 0
    fi
...
**Key points:**
- Default is NO (safe default — don't surprise the user)
- Skipped in silent mode and via `APPNAME_SKIP_REBOOT=true`
...
# umask 077  # ONLY enable if script generates secret files (passwords, tokens)
#            # Do NOT enable for scripts that create shared directories, web roots,
#            # or any path requiring group/world access — umask 077 makes mkdir -p
#            # create parent directories as 700, breaking traversal even if you
#            # chmod the final directory afterward.
...
- [ ] (Optional) `SPINNER_CHARS` declared in separate block after standard Unicode symbols
- [ ] (Optional) `run_with_spinner()` — if used, must match skill verbatim
